...
/*   Updated: 2023/11/22 15:04:22 by pchapuis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	contact_valid(t_raycasting *ray, t_data *img, t_pos_int *pos)
{
	if (img->map.map[pos->y][pos->x] == '1')
	{
		if (ray->side == 1)
		{
			if (img->map.map[pos->y - ray->step_y][pos->x] == 'd')
				ray->side = 5;
		}
		else if (ray->side == 0)
		{
			if (img->map.map[pos->y][pos->x - ray->step_x] == 'd')
				ray->side = 4;
		}
		return (1);
	}
	else if (img->map.map[pos->y][pos->x] == 'D')
	{
		ray->side += 2;
		return (1);
	}
	...
	t_data *img, t_pos_int *pos)
{
	...
	while (contact != 1)
	{
		...
		contact = contact_valid(ray, img, pos);
	}
}
