...
/*   Updated: 2023/11/22 17:39:17 by pchapuis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fill_path(t_data *img)
{
	...
	if (check_name(img->map.path_door, ".xpm") == 0)
		return (error_config('X'), 5);
	if (check_text_path(img, img->map.path_door, &img->map.door))
		return (error_config('T'), 5);
	return (0);
}
